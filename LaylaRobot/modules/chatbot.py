...
import html
import requests
import LaylaRobot.modules.sql.kuki_sql as sql

from time import sleep
from telegram import ParseMode
from LaylaRobot import dispatcher, updater, SUPPORT_CHAT
...
def add_chat(update, context):
    ...
    if not is_kuki:
        ...
        msg.reply_text("ChatBot AI successfully enabled for this chat!")
        message = (
        ...
        return message
    msg.reply_text("Oda AI is already enabled for this chat!")
    return ""
...
def rem_chat(update, context):
    ...
    is_kuki = sql.is_kuki(chat.id)
    if not is_kuki:
        msg.reply_text("ChatBot AI isn't enabled here in the first place!")
        return ""
    sql.rem_kuki(chat.id)
    msg.reply_text("ChatBot AI disabled successfully!")
    message = (
        f"<b>{html.escape(chat.title)}:</b>\n"
        f"AI_DISABLED\n"
        f"<b>Admin:</b> {mention_html(user.id, html.escape(user.first_name))}\n"
...
def kuki_message(update, context):
    reply_message = message.reply_to_message
    if message.text.lower() == "wibu":
        return True
    if reply_message:
...
__help__ = """
Chatbot utilizes the Kuki API and allows Oda to talk and provides a more interactive group chat experience.
──「 *Admins Only*: 」──
